 ...
 */
public class FPSFPV313 extends Output {
    ...
    String arrivalAirport = "";
        ...
        public FPSFPV313(Config config) {
            super(config);
...
for (int j = 0; j < childNodesX.getLength(); j++) {
    ...
    if (nNodeX.getNodeType() == Node.ELEMENT_NODE) {
        ...
        NodeList descendants = trackZ.getElementsByTagName("nxce:arrivalPoint");
        if (descendants.getLength() > 0) {
            ...
            for (int k = 0; k < descendants.getLength(); k++) {
                ...
                if (descendantNode.getNodeType() == Node.ELEMENT_NODE) {
                    Element descendantElement1 = (Element) descendantNode;
                    arrivalAirport = descendantElement1.getElementsByTagName("nxce:airport").item(0).getTextContent();
                    row = row + "," + descendantElement1.getElementsByTagName("nxce:airport").item(0).getTextContent();
