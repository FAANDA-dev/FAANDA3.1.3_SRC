...
public class FPS321 extends Output {
    ...
    boolean FIX = false;
        ...
        public FPS321(Config config) {
            ...
            outputFile = new File("./log/FlightPlansData77.csv");
             // create the CSV file
             try {
...
//departure point airport *****************************************
  // Process child node 'arrivalPoint'
   String expressionChildX8 = ".//nxce:arrivalPoint";
   NodeList childNodesX8 = (NodeList) xPath.compile(expressionChildX8).evaluate(eElement, XPathConstants.NODESET);

   // Check if any descendants are found
   boolean hasFix2 = false;
 // Iterate over child nodes
for (int j = 0; j < childNodesX8.getLength(); j++) {
    Node nNodeX8 = childNodesX8.item(j);
    if (nNodeX8.getNodeType() == Node.ELEMENT_NODE) {
        Element trackZ = (Element) nNodeX8;
