 ...
 */
public class FPSFPV310 extends Output {
    ...
    String FIX = "";
        ...
        public FPSFPV310(Config config) {
            super(config);
                 
                   
            
            outputFile = new File("./log/FlightPlansData12.csv");
             // create the CSV file
             try {
...
for (int j = 0; j < childNodesX.getLength(); j++) {
  ...
  // Process child node 'qualifiedAircraftId'
  String expressionChildX1 = ".//nxcm:qualifiedAircraftId";
  NodeList childNodesX1 = (NodeList) xPath.compile(expressionChildX1).evaluate(eElement, XPathConstants.NODESET);

  // Check if any descendants are found
  boolean hasDescendants1 = false;
// Iterate over child nodes
for (int j = 0; j < childNodesX1.getLength(); j++) {
   Node nNodeX1 = childNodesX1.item(j);
   if (nNodeX1.getNodeType() == Node.ELEMENT_NODE) {
       Element track = (Element) nNodeX1;

       // Check if descendants exist for the child node
       NodeList descendants1 = track.getElementsByTagName("nxce:fix");
       if (descendants1.getLength() > 0) {
           hasDescendants1 = true;

           // Iterate over descendants
           for (int k = 0; k < descendants1.getLength(); k++) {
               Node descendantNode1 = descendants1.item(k);
               if (descendantNode1.getNodeType() == Node.ELEMENT_NODE) {
                   Element descendantElement1 = (Element) descendantNode1;

                   // Access further down nodes within the descendant element
                   NodeList furtherDescendants1 = descendantElement1.getElementsByTagName("nxce:longtitudeDMS");
...
System.out.println("Fix2: " + hasDescendants1);
