 ...
 */
public class FPS323 extends Output {
    ...
    boolean FIX = false;
        ...
        public FPS323(Config config) {
            super(config);
                 
                   
            
            outputFile = new File("./log/FlightPlansData3333.csv");
             // create the CSV file
             try {
...
for (int j = 0; j < childNodesX30.getLength(); j++) {
   ...
   if(has_Arr == false)
    ...
    else{                  row = row + "," + track3.getElementsByTagName("nxce:airport").item(0).getTextContent();
         }
