...
public class F552DR extends Output {
    private static final Logger logger = LoggerFactory.getLogger("file");
    private final File outputFile;
    private final Set<String> uniqueRecords;
    String row = "";

    public F552DR(Config config) {
        super(config);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (eElement.getAttribute("msgType").equals("trackInformation")) {
                    ...
                    String timeAtPosition = eElement.getElementsByTagName("nxcm:timeAtPosition").item(0).getTextContent();

                    String combinedKey = combineKey(aircraftId, timeAtPosition);
                    if (uniqueRecords.contains(combinedKey)) {
                        continue; // Skip duplicate record
                    }

                    String row = aircraftId + "," + timeAtPosition;
                    returnString.append(row).append("\n");

                    uniqueRecords.add(combinedKey);
                }
            }
        }
    } catch (Exception e) {
...
private String combineKey(String aircraftId, String timeAtPosition) {
    return aircraftId + "_" + timeAtPosition;
}
